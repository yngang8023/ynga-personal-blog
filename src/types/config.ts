...
export interface AutoScanPublicDirConfig {
	enable: boolean; // 是否自动扫描 public 目录中的图片
	desktopDir?: string; // 桌面端图片目录，相对于 public
	mobileDir?: string; // 移动端图片目录，相对于 public
	pattern?: string; // 文件匹配模式，默认 webp
}
...
export interface SiteConfig {
	...
	banner: {
		src: ResponsiveImageSource; // 支持单个图片、图片数组或分别设置桌面端和移动端图片
...
export interface ExternalLinkConfig {
	enable: boolean; // 是否启用外链访问确认弹窗；关闭后外链将直接在新标签页打开
	logo?: string; // 弹窗中展示的站点 Logo，可留空以回退到导航栏 Logo / 图标 / 头像
}
