...
export function getAnchorScrollOffset(): number {
	...
	const cssOffset = Number.parseFloat(
		rootStyles.getPropertyValue("--sidebar-sticky-top"),
	);
	...
	const stickyGap =
		Number.parseFloat(rootStyles.getPropertyValue("--sidebar-sticky-gap")) ||
		16;

	return navbarHeight + stickyGap;
}
...
export function createHeadingObserver(
	...
	const defaultRootMargin = `-${getAnchorScrollOffset()}px 0px -80% 0px`;
	const { rootMargin = defaultRootMargin, threshold = 0 } = options;
