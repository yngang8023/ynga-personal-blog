...
export class TOCManager {
	...
	constructor(config: TOCConfig) {
		...
		this.scrollOffset = config.scrollOffset || getAnchorScrollOffset();
		this.emptyStateHTML = config.emptyStateHTML ?? "";
	}
	...
	private getEmptyStateHTML(): string {
		return (
			this.emptyStateHTML ||
			`<div class="text-center py-8 text-gray-500 dark:text-gray-400"><p>${i18n(I18nKey.tocEmpty)}</p></div>`
		);
	}
