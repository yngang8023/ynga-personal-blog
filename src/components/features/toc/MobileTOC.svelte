<script lang="ts">
	...
	const setupIntersectionObserver = () => {
		...
		observer = new IntersectionObserver(
			...
			{
				rootMargin: `-${getAnchorScrollOffset()}px 0px -80% 0px`,
				threshold: 0,
			},
		);
...
>
	...
	{#if isHomePage}
		{#if postItems.length === 0}
			<div class="mobile-empty-state">
				...
				<p class="mobile-empty-title">{i18n(I18nKey.postListEmpty)}</p>
			</div>
		{:else}
	...
	{:else if tocItems.length === 0}
		<div class="mobile-empty-state">
			...
			<p class="mobile-empty-title">{i18n(I18nKey.tocEmpty)}</p>
		</div>
	{:else}
...
<style>
	...
	.mobile-empty-icon {
		...
		background:
			linear-gradient(
				135deg,
				oklch(0.97 0.03 var(--hue)),
				oklch(0.93 0.06 var(--hue))
			);
		color: var(--primary);
		box-shadow: 0 12px 30px rgba(217, 153, 0, 0.16);
	}
