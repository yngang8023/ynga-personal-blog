...
const warnedMissingAnimeFiles = new Set<string>();

export function loadAnimeData(filename: string): AnimeItem[] {
	const dataPath = path.join(process.cwd(), `src/data/${filename}`);

	if (!fs.existsSync(dataPath)) {
		if (!warnedMissingAnimeFiles.has(dataPath)) {
			warnedMissingAnimeFiles.add(dataPath);
			const commandHint = filename.includes("bilibili")
				? "pnpm update-bilibili"
				: filename.includes("bangumi")
					? "pnpm update-bangumi"
					: "pnpm update-anime";
			console.warn(
				`[Anime] Data file not found: ${dataPath}. Run \`${commandHint}\` to generate local debug data.`,
			);
		}
		return [];
	}
